...
const DEFAULT_OLLAMA_BIN = "ollama";
const COMMON_POSIX_OLLAMA_DIRS = [
  "/opt/homebrew/bin",
  "/usr/local/bin",
  "/usr/bin",
];
...
async function resolveBareCommand(
  ...
  const searchDirs = uniqueStrings([
    ...
    ...(process.platform === "win32" ? [] : COMMON_POSIX_OLLAMA_DIRS),
  ]);
...
export async function resolveOllamaBin(
  requestedBin: string | undefined,
  baseEnv: NodeJS.ProcessEnv = process.env
): Promise<string> {
  const normalized = requestedBin?.trim() || DEFAULT_OLLAMA_BIN;
  ...
  if (path.basename(normalized) === DEFAULT_OLLAMA_BIN) {
    return (await resolveBareCommand(DEFAULT_OLLAMA_BIN, baseEnv)) ?? resolvedPath;
  }
